...
class LoRA_EnsembleRSSM(common.Module):
    def __init__(
        self,
        action_free=False,
        lora_r=2,
        ...
        self._lora_r = lora_r
        self._cell = LoRA_GRUCell(self._deter, norm=True, lora_r=lora_r)
    ...
    def obs_step(self, prev_state, prev_action, embed, is_first, sample=True):
        ...
        x = self.get("obs_out", common.LoRADense, self._hidden, self._lora_r)(x)
    ...
    def img_step(self, prev_state, prev_action, sample=True):
        ...
        x = self.get("img_in", common.LoRADense, self._hidden, self._lora_r)(x)
    ...
    def _suff_stats_ensemble(self, inp):
        ...
        for k in range(self._ensemble):
            x = self.get(f"img_out_{k}", common.LoRADense, self._hidden, self._lora_r)(inp)
    ...
    def _suff_stats_layer(self, name, x):
        if self._discrete:
            x = self.get(name, common.LoRADense, self._stoch * self._discrete, self._lora_r)(x)
            logit = tf.reshape(x, x.shape[:-1] + [self._stoch, self._discrete])
            return {"logit": logit}
        else:
            x = self.get(name, common.LoRADense, 2 * self._stoch, self._lora_r)(x)
    ...
    def load(self, filename, verbose=True):
        ...
        for k in range(len(self.variables)):
            if 8 not in self.variables[k].shape:
                indices.append(k)
        if verbose:
            print(f"Load checkpoint with {amount} tensors and {count} parameters.")
        for idx_to, idx_from in zip(indices, range(len(values))):
            self.variables[idx_to].assign(values[idx_from])
        # tf.nest.map_structure(lambda x, y: x.assign(y), self.variables, values)

        print("123")


class LoRA_GRUCell(tf.keras.layers.AbstractRNNCell):
    def __init__(self, size, norm=False, act="tanh", update_bias=-1, lora_r=1, **kwargs):
        super().__init__()
        self._size = size
        self._act = get_act(act)
        self._norm = norm
        ...
        self._lora_r = lora_r
        self._layer = common.LoRADense(3 * size, lora_r, use_bias=norm is not None, **kwargs)
        if norm:
